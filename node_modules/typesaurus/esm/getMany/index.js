 ...
 */
export default async function getMany(collection, ids, onMissing = (id) => {
    ...
    return firestoreSnaps
        ...
        if (!firestoreSnap.exists) {
            if (onMissing === 'ignore') {
                ...
                return doc(ref(collection, firestoreSnap.id), onMissing(firestoreSnap.id), a.getDocMeta(firestoreSnap));
            }
        }
        const firestoreData = firestoreSnap.data();
        const data = firestoreData && wrapData(a, firestoreData);
        return doc(ref(collection, firestoreSnap.id), data, a.getDocMeta(firestoreSnap));
    })
        .filter((doc) => doc != null);
}
